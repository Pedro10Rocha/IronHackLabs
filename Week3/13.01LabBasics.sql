...
select `date`, count(loan_id), distinct duration from loan where `date` like '9712%' group by duration limit 10;
...
select account_id, concat(`amount`,' ', `amount`) as 'amount', floor(sum(amount)) as total_amount from trans where account_id = 396 group by `type` limit 1;